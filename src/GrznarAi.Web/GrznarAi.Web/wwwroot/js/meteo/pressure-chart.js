// Funkce pro vykreslení grafu atmosférického tlaku
window.renderPressureChart = function (elementId, categories, minData, avgData, maxData) {
    console.log("Vykreslování grafu atmosférického tlaku do elementu: " + elementId);
    ...
    if (window.pressureChart) {
        try {
            ...
            console.warn("Nepodařilo se zničit předchozí graf atmosférického tlaku:", e);
        }
    ...
    let chartTitle = 'Historie atmosférického tlaku';
    let xaxisTitle = 'Čas';
    ...
    if (categories && categories.length > 0) {
        ...
        if (sampleCategory.includes(':00')) {
            chartTitle = 'Hodinová historie atmosférického tlaku';
            xaxisTitle = 'Hodina';
        } else if (sampleCategory.includes(' - ')) {
            chartTitle = 'Týdenní historie atmosférického tlaku';
            xaxisTitle = 'Týden';
        } else if (sampleCategory.includes('.') && !sampleCategory.includes('.20')) {
            chartTitle = 'Denní historie atmosférického tlaku';
            xaxisTitle = 'Den';
        } else if (sampleCategory.includes('.20')) {
            chartTitle = 'Měsíční historie atmosférického tlaku';
            xaxisTitle = 'Měsíc';
        }
    }
    ...
    try {
        ...
        window.pressureChart = new ApexCharts(chartElement, {
            chart: {
                type: 'line',
                height: 380,
                toolbar: { show: true },
        ...
        console.log("Graf atmosférického tlaku byl úspěšně vykreslen");
        
    } catch (error) {
        console.error("Chyba při vykreslování grafu atmosférického tlaku:", error);
    }
}; 