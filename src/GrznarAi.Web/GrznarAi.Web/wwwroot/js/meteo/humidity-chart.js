// Funkce pro vykreslení grafu vlhkosti
window.renderHumidityChart = function (elementId, categories, minData, avgData, maxData) {
    console.log("Vykreslování grafu vlhkosti do elementu: " + elementId);
    ...
    if (window.humidityChart) {
        try {
            ...
            console.warn("Nepodařilo se zničit předchozí graf vlhkosti:", e);
        }
    ...
    let chartTitle = 'Historie vlhkosti';
    let xaxisTitle = 'Čas';
    ...
    if (categories && categories.length > 0) {
        ...
        if (sampleCategory.includes(':00')) {
            chartTitle = 'Hodinová historie vlhkosti';
            xaxisTitle = 'Hodina';
        } else if (sampleCategory.includes(' - ')) {
            chartTitle = 'Týdenní historie vlhkosti';
            xaxisTitle = 'Týden';
        } else if (sampleCategory.includes('.') && !sampleCategory.includes('.20')) {
            chartTitle = 'Denní historie vlhkosti';
            xaxisTitle = 'Den';
        } else if (sampleCategory.includes('.20')) {
            chartTitle = 'Měsíční historie vlhkosti';
            xaxisTitle = 'Měsíc';
        }
    }
    ...
    try {
        ...
        window.humidityChart = new ApexCharts(chartElement, {
            chart: {
                type: 'line',
                height: 380,
                toolbar: { show: true },
        ...
        console.log("Graf vlhkosti byl úspěšně vykreslen");
        
    } catch (error) {
        console.error("Chyba při vykreslování grafu vlhkosti:", error);
    }
}; 